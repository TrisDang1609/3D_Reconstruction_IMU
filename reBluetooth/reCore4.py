...
class BLUETOOTH_DATA_READER:
    ...
    # [ ... _notification_enabler method remains largely unchanged, ensures notifies start for connected devices ... ]
    ...
    def _parse_data_string(self, data_string):
        ...
        try:
            # Example format: "[DATA] IMU1 1713866469.962345: 0.1 0.2 ... 0.7"
            ...
            if len(values) == 7:
                ...
                log.warning(f"Parsed incorrect number of values ({len(values)}) from: {data_string}")
                return None, None
        except Exception as e:
                ...
                try:
                    for i, name in enumerate(self.target_device_order):
                        ...
                        if timestamp is not None and values is not None:
                            ...
                            # Handle parsing error - maybe add placeholders?
                            log.warning(f"Could not parse data for {name}, skipping CSV row values for it.")
                            if i == 0: csv_row.append("PARSE_ERROR")
                            csv_row.extend(["PARSE_ERROR"] * 7) # Add placeholders
                    ...
                    # Write to CSV if row is complete (1 timestamp + 6*7 values)
                    if len(csv_row) == 1 + (len(self.target_device_order) * 7):
                        ...
                        log.warning(f"CSV row has incorrect length ({len(csv_row)}), expected {1 + len(self.target_device_order) * 7}. Skipping write.")
    ...
    def _open_csv_file(self):
        ...
        try:
            ...
            for name in self.target_device_order: # Use the defined order
                for i in range(1, 8): # 7 values per IMU
                    header.append(f"{name}_Val{i}")
...
if __name__ == "__main__":
    ...
    csv_output_filename = "synchronized_imu_data.csv" # Define CSV filename
    ...
    try:
        ...
        if len(found_devices_map) == len(target_devices_names) and all(name in found_devices_map for name in target_devices_names):
            ...
            run_reader = True
        elif found_devices_map:
             log.warning(f"--- Found {len(found_devices_map)} devices, but not all {len(target_devices_names)} required target devices ({target_devices_names}). Found: {list(found_devices_map.keys())}. Synchronized reading cannot start. ---")
        else:
        ...
        # Cleanup is handled within reader's run/stop methods now
        ...
        if not main_loop.is_closed():
             if not main_loop.is_running():
                  ...
                  try:
                      # Simplified cleanup for potentially running loop
