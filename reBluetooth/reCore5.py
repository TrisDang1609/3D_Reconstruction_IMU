...
class BLUETOOTH_DATA_READER:
    ...
    def __init__(self, devices_dict,
        ...
        Args:
            devices_dict (dict): Dictionary {device_name: mac_address}. Should contain IMU1..IMU6 for synchronized read.
        ...
        # Ensure target devices are present for synchronized read
        self.target_device_order = ["IMU1", "IMU2", "IMU3", "IMU4", "IMU5", "IMU6"]
        if not all(name in devices_dict for name in self.target_device_order):
             log.warning(f"devices_dict is missing one or more required devices from {self.target_device_order}. Synchronized read might not work.")
             # Decide if this should be a hard error?
             # raise ValueError(f"devices_dict must contain all of {self.target_device_order} for synchronized mode.")


        self.devices_dict = devices_dict
        self.device_names = list(devices_dict.keys()) # All devices passed in
        self.num_target_devices = len(self.target_device_order) # We specifically care about these 6
    ...
    def _parse_data_string(self, data_string):
        ...
        try:
            # Example format: "[DATA] IMU1 1713866469.962345: 0.1 0.2 ... 0.6" (Now 6 values)
    ...
    async def _data_processor(self):
        """
        Phase 2: Loop to wait for data in all 6 target queues (IMU1-IMU6)
                 and process synchronized data sets, writing to CSV.
        """
        try:
            ...
            log.info(f"--- PHASE 2: Starting Data Processor (Synchronized Mode for {self.target_device_order}) ---")
            log.info(f"Writing data to: {self.csv_filename}")

            # ***** MODIFIED: Expected number of values per IMU for CSV *****
            expected_values_per_imu = 6
            expected_csv_columns = 1 + (len(self.target_device_order) * expected_values_per_imu)

            while self._running and self.current_phase == ReaderPhase.PHASE_2_READING_DATA:
                # --- Check if ALL target devices are currently notifying ---
                all_targets_notifying = True
                for name in self.target_device_order:
                    if self.device_states.get(name) != DeviceStatus.CONNECTED_NOTIFYING:
                        all_targets_notifying = False
                        log.warning(f"Waiting for target device {name} to be CONNECTED_NOTIFYING (current: {self.device_states.get(name)}).")
                        break # No need to check others if one is not ready

                if not all_targets_notifying:
                    # Optional: Check if recovery is possible
                    can_recover = any(self.device_states.get(name) in
                                      [DeviceStatus.WAITING_RETRY, DeviceStatus.CONNECTING, DeviceStatus.CONNECTED_NO_NOTIFY]
                                      for name in self.target_device_order if self.device_states.get(name) != DeviceStatus.CONNECTED_NOTIFYING)
                    if not can_recover and self._running:
                         log.error(f"Not all target devices ({self.target_device_order}) are notifying and recovery seems impossible. Stopping Data Processor.")
                         break # Exit processor loop
                    await asyncio.sleep(1.0) # Wait longer if not all target devices are ready
                    continue

                # --- Check if all target queues have data ---
                all_queues_ready = True
                queue_sizes = {}
                for name in self.target_device_order:
                    q = self.device_queues.get(name)
                    # Check state again to prevent race condition after long sleep
                    if self.device_states.get(name) != DeviceStatus.CONNECTED_NOTIFYING:
                         log.warning(f"Target device {name} stopped notifying while checking queues.")
                         all_queues_ready = False
                         break
                    if not q or q.empty():
                        all_queues_ready = False
                        # log.debug(f"Queue for {name} is empty. Waiting...")
                        break # At least one required queue is empty
                    queue_sizes[name] = q.qsize() # For debugging

                if not all_queues_ready:
                    await asyncio.sleep(0.01) # Sleep briefly and check again
                    continue

                # --- All target queues have data, retrieve synchronized set ---
                synchronized_data = {}
                csv_row = [] # Prepare row for CSV
                first_timestamp = None # Use timestamp from the first device in order

                try:
                    for i, name in enumerate(self.target_device_order):
                        # We know the queue is not empty due to the check above
                        data_string = await self.device_queues[name].get()
                        self.device_queues[name].task_done()
                        synchronized_data[name] = data_string

                        # Parse data for CSV
                        timestamp, values = self._parse_data_string(data_string)
                        if timestamp is not None and values is not None:
                            if i == 0: # First device's timestamp
                                first_timestamp = timestamp
                                csv_row.append(f"{timestamp:.6f}") # Add timestamp as first column
                            csv_row.extend(f"{v:.6f}" for v in values) # Add values for this device
                        else:
                            # Handle parsing error - add placeholders
                            log.warning(f"Could not parse data for {name}, adding placeholders to CSV row.")
                            if i == 0: csv_row.append("PARSE_ERROR_TS") # Placeholder for timestamp if first fails
                            # ***** MODIFIED: Add 6 placeholders *****
                            csv_row.extend(["PARSE_ERROR_VAL"] * expected_values_per_imu)

                    # --- Process the synchronized data (Log and Write to CSV) ---
                    log.info("--- Synchronized Data Set Received ---")
                    for name in self.target_device_order:
                         log.info(f"  {synchronized_data.get(name, f'Error: Missing data for {name}')}")
                    log.info("--------------------------------------")

                    # ***** MODIFIED: Check against expected CSV columns (1 + num_devices * 6) *****
                    if len(csv_row) == expected_csv_columns:
                        ...
                        # ***** MODIFIED: Update log message for expected length *****
                        log.warning(f"CSV row has incorrect length ({len(csv_row)}), expected {expected_csv_columns}. Skipping write. Row data: {csv_row}")


                except asyncio.CancelledError:
                     log.info("Data retrieval cancelled during synchronized get.")
                     raise # Re-raise cancellation
                except Exception as e:
                     log.exception(f"Error during synchronized data retrieval/processing: {e}")
                     # Handle error, e.g., clear queues? Skip this cycle?
    ...
    def _open_csv_file(self):
        ...
        try:
            ...
            # Create header row
            header = ["Timestamp"]
            for name in self.target_device_order: # Use the defined order
                # ***** MODIFIED: Generate headers Val1 to Val6 *****
                for i in range(1, 7): # 6 values per IMU (AccelX,Y,Z, GyroX,Y,Z)
                    header.append(f"{name}_Val{i}") # Or be more specific: f"{name}_AccX", f"{name}_AccY", etc.
...
if __name__ == "__main__":
    log.info("--- Starting Bluetooth IMU Data Collector (Two-Phase, 6-Value IMU) ---")

    # Ensure these names match the devices_dict keys and the desired sync order
    target_devices_names = [ "IMU1", "IMU2", "IMU3", "IMU4", "IMU5", "IMU6"]
    log.info(f"Target devices for synchronized reading: {target_devices_names}")
    csv_output_filename = "synchronized_imu_data_6val.csv" # Define CSV filename (updated name)
    ...
    # --- Run Event Loop ---
    # Use asyncio.run() for simpler main loop management if Python 3.7+
    # If needing lower level control or compatibility, keep the manual loop setup.
    main_loop = None
    try:
    ...
    async def main():
        ...
        try:
            ...
            # Important: Ensure found_devices_map uses the EXACT names "IMU1".."IMU6" as keys
            found_devices_map = await detector.OUTSYSTEM_SCANNING()

            # 3. Initialize Data Reader if ALL target devices were found
            num_targets_found = sum(1 for name in target_devices_names if name in found_devices_map)

            if num_targets_found == len(target_devices_names):
                log.info(f"--- Found all {len(target_devices_names)} target devices: {found_devices_map} ---")
                ...
                run_reader = True
                 ...
                 log.warning(f"--- Found {num_targets_found}/{len(target_devices_names)} required target devices. Required: {target_devices_names}. Found: {list(found_devices_map.keys())}. Synchronized reading cannot start. ---")
            ...
            if run_reader and reader:
                 ...
                 log.info("--- Reader will not start as not all target devices were found or initialized. ---")
            ...
            # Cleanup is largely handled by reader.stop()
