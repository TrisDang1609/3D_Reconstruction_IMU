...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("Silicon Labs BLE with IMU data streaming - MG24_3");
...
static const uint8_t advertised_name[] = "IMU3";  // Changed name for unique identification
...
/**************************************************************************/ /**
 * Sends a BLE notification with raw IMU data to the client if notifications are enabled 
 *****************************************************************************/
static void send_imu_notification() {
  ...
  // Create a buffer to hold only raw IMU data (24 bytes total - 6 floats)
  uint8_t buffer[24];
  
  // Copy the raw accelerometer and gyroscope values to the buffer
  memcpy(&buffer[0], &aX, 4);
  memcpy(&buffer[4], &aY, 4);
  memcpy(&buffer[8], &aZ, 4);
  memcpy(&buffer[12], &gX, 4);
  memcpy(&buffer[16], &gY, 4);
  memcpy(&buffer[20], &gZ, 4);
  
  // Send the notification with raw IMU data
  sl_status_t sc = sl_bt_gatt_server_notify_all(notify_characteristic_handle,
                                               sizeof(buffer),
                                               (const uint8_t *)&buffer);
  if (sc == SL_STATUS_OK) {
    // Print raw IMU values in decimal format
...
static void ble_initialize_gatt_db() {
  ...
  sc = sl_bt_gattdb_add_uuid128_characteristic(gattdb_session_id,
                                               ...
                                               sl_bt_gattdb_fixed_length_value,
                                               24,                              // max length - 24 bytes (6 floats × 4 bytes)
